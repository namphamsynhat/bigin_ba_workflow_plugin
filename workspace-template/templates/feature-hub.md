...
## Signal Log
<!-- One row per signal, in the order it landed. Never renumber or delete a row — a signal's # is permanent, like a BR-### number. When a later signal conflicts with or supersedes an earlier one, add the NEW row and update the OLD row's Status + Notes to point at it; don't rewrite history in place.
