...
**Platform is a supplied fact, exactly like the slug and the in-scope UCs.** The orchestrator read `platform:` once, at Stage 1, from `_bigin/system/project.md` (absent → `web`) and resolved it for the whole run; you take the value as given and write it into your brief verbatim. You do **not** open the project config to check it, you do not derive it from what the UCs describe, and you do not resolve a design engine at all — the method layer was decided once upstream at Stage 1, and there is no render engine in a design run to resolve (rendering is `/bigin-render-design`, invoked by a human later). Re-deciding any of it here is how one product ends up with two navigation shells. It shapes what the screens will look like (the regions vocabulary, the nav shell, how many surfaces a step needs), which is the screens-writing worker's job, not yours.
