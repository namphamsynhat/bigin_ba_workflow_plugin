...
- `design-core.md` § Paths · `design-screens.md` § The UX spec · `design-grounding.md` § Grounding ·
  `design-actor-scope.md` § Actor scope · `design-navigation.md` § The navigation map. Those five files
  and nothing else — do not open `design-conventions.md`, which is only a map.
...
The orchestrator supplies: the feature slug, the UCs in scope this run (NEW, or CHANGED with old→new version), **the platform** (`web | mobile | both`), whether a UX spec already exists for this feature (path, if so), and this run's nav-map file path (already bootstrapped or loaded by Stage 2 — you read it, you never bootstrap it). There is no design system and no token file in this pipeline: an element names a **semantic role** from a closed list of ten, and colour, type, spacing, and components come from the design team later or are bound at render time.

**Platform is a supplied fact, exactly like the slug and the in-scope UCs.** The orchestrator read `platform:` once, at Stage 1, from `_bigin/system/project.md` (absent → `web`) and resolved it for the whole run; you take the value as given and write it into your brief verbatim. You do **not** open the project config to check it, you do not derive it from what the UCs describe, and you do not resolve a design engine at all — the method layer was decided once upstream at Stage 1, and there is no render engine in a design run to resolve (rendering is `/bigin-render-design-od`, invoked by a human later). Re-deciding any of it here is how one product ends up with two navigation shells. It shapes what the screens will look like (the regions vocabulary, the nav shell, how many surfaces a step needs), which is the screens-writing worker's job, not yours.
...
6. **The hub's `## Pain Points`** — every row **not** resolved, reported `PP-###` and statement **verbatim**, in the client's own words. These are what the worker's `## 4` flows have to fix, and every one of them has to end up either named in some flow's `Resolves` cell or turned into a `## 6` question. Report them as facts; never propose which flow resolves which, and never mark one resolved — the register is read-only for you and for the worker alike.
...
9. **Sibling patterns — by `Grep`, not by reading.** One `Grep` over `{ux_dir}` for the pattern words this feature actually needs (`queue`, `approval`, `wizard`, whichever apply), and open **at most one** matching spec's `## 2 Screen Inventory` to see how it was solved. A match is a ground-2a candidate; note it, do not assume it fits. **Never open every `UX-*.md`** — at two features that is a habit and at ten it is nine specs in context before this feature's brief has a line, and the ten-feature project is the one where the cost lands.
...
- **Never decide the final screen boundary, assign a semantic role, define a state, write a flow, or fill a nav entry.** You propose a candidate list per Part 2's mechanical rule; the screens-writing worker confirms, adjusts, or discards it. Flows in particular are the worker's (`3-screens.md` Part 4c) — you report which pain points are open, never which journey answers one.
- **Never treat a pain point as a reason for a screen.** A `PP-###` is ground 1b: it shapes ordering, emphasis, defaults, and nav placement on a screen a UC already asked for, and it grounds nothing existing on its own. A candidate screen whose only justification is a pain point is not a candidate — report the pain point and let the worker raise the question.
...
- **Never touch a UC, BR, entity, or pain-point file, and never touch the nav map or another feature's anything.** Read-only, this feature only.
...
existing_vocabulary:  nav branches: <existing ids, comma list>
                      (no tokens and no components — this pipeline holds no design system; an
                      element names one of the ten semantic roles, or none)
pain_points_open:     PP-### — <the statement, VERBATIM, in the client's own words>
                      (one per line, or "none open". Facts only: never which flow resolves one,
                      never a resolved verdict, never a screen proposed on a pain point's strength)
