...
4. **`## Open Questions / Gates`**: mirror every open UC's `## 5` Still open line and every open BR's `## Open Questions` line here, worded identically to the source — never paraphrased into a second version of the question. Remove a line whose source question was resolved (moved to a Decision log / ticked off) since the last refresh. A settled decision-log row is not an open item and does not belong here.
5. **`## Changelog`**: append one line — date, what changed, which run/stage touched it — never rewrite an existing line.
6. **Reconcile, don't assume.** Every one of the above is a re-derivation from the artifact's current disk state, not an incremental patch — read the UC/BR fresh, write what it says now. This is what makes a retry or a resumed run safe: setting an already-correct field again costs nothing.
