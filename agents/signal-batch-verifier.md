...
7. **Rationale batching and marking** — at most one open question per note about missing "not stated" reasons, **and** every `requirement`/`feedback` row whose `Why` is `not stated` carries exactly one `rationale: in question` / `rationale: non-blocking` marker in its `Notes` (`3-filing.md` § Step 5). Both halves are mechanical: count the questions, read the cells. Never judge whether the call was the right one — that is filing's, and it is not reviewable from here. Report every unmarked row by number: `/bigin-transform-signal` passes those through **unblocked** (`2-qualification.md` § Gate 1), so an unmarked row is a decision nobody recorded rather than a requirement anybody is holding.
