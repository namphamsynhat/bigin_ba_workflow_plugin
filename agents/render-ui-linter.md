...
- **Stage 4 Part 3, once per rendered spec**, immediately after that spec's `render-ui-designer`
  returns.
...
`{nav_map_file}` and `{tokens_file}` paths, and the designer's own report — which you treat as a
**claim to verify**, never as a finding. A designer reporting `traceability: clean` is exactly the
case to scan hardest; it is the report that reads correct.

## Part 1 — the traceability scan (deterministic, and it runs first)
...
## Part 2 — the fidelity verification
...
verdict:       PASS | RE-RENDER (<N> findings for the designer) | BLOCKED (<why>)
```

`verdict: PASS` is what lets the orchestrator write the `## 8` row. Anything else means no row is
written for the screens that failed — a render recorded as complete with one screen wrong is the
