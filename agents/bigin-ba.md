...
7. `prototype-design` — **[Load]** flows/screens/states for an approved feature, traceable to its use cases.
...
- **Stop at the migration boundary.** `enrich-feature`, `approve-fr`, `prototype-design`, and `consolidate-prd` still read the pre-migration `.bigin/features/` layout **and still key on the retired `FR-###` artifact**, while `bigin-transform-signal` writes `01-Requirements/_ucs/UC-<NNN> …`. Nothing bridges them yet. When the next stage would be `enrich-feature`, say so and stop rather than run a stage that reads the wrong paths and reports finding nothing. `_bigin/conventions/conventions.md` § Reconciliation notes lists what each of the four needs.
