...
- **Stop at the migration boundary — but design is on the near side of it.** `enrich-feature`, `approve-fr`, and `consolidate-prd` still read the pre-migration `.bigin/features/` layout **and still key on the retired `FR-###` artifact**, while `bigin-transform-signal` writes `01-Requirements/_ucs/UC-<NNN> …`. Nothing bridges those three yet: when the next stage would be `enrich-feature`, say so and stop rather than run a stage that reads the wrong paths and reports finding nothing. **`bigin-generate-design` is migrated and safe to run** — it reads `_ucs/` directly, so after `bigin-transform-signal` the design stage is a real next step, not a dead end. `_bigin/conventions/conventions.md` § Reconciliation notes lists what each remaining skill needs.
